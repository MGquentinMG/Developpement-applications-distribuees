...
export default function UserProfilePage() {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto flex flex-col md:flex-row min-h-screen bg-white dark:bg-[#121212] transition-colors duration-300">
      ...
      <main className="flex-1 w-full max-w-2xl min-h-screen bg-white dark:bg-[#121212] border-x border-gray-100 dark:border-gray-800 transition-colors duration-300">
        ...
        {!loading && profile && (
          <>
                  ...
                  {!isOwnProfile && currentUser && (
                    <div className="flex gap-3 mb-4">
                      <button
                        ...
                        className={`px-5 py-2 rounded-full font-semibold text-xs transition-colors cursor-pointer border-none ${
                          isFollowing
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-5 py-2 rounded-full font-semibold text-xs bg-[#F4F2F9] dark:bg-[#2A2A40] text-[#5A4B81] dark:text-[#D0C9E8] hover:bg-[#e4dff0] dark:hover:bg-[#3A3A55] transition-colors cursor-pointer border-none"
                      >
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-800 transition-colors">
              ...
              {posts.map((post) => (
                <div key={post._id} className="bg-white dark:bg-[#1A1A2E] p-4 flex gap-3 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-700 shrink-0 overflow-hidden">
                    <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-sm text-black dark:text-[#F9F9FB]">@{profile.username}</p>
                    <p className="text-[10px] text-gray-400 mb-1">{timeAgo(post.createdAt)}</p>
                    <p className="text-gray-800 dark:text-[#D0C9E8] text-sm mt-1">{post.content}</p>
                    {post.image && (
