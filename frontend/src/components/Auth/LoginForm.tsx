  ...
  return (
    <div className="bg-white dark:bg-[#1A1A2E] rounded-3xl shadow-xl w-full max-w-[500px] p-10 pt-12 relative transition-colors duration-300">
      ...
      <div className="text-center mb-10">
        <h2 className="text-[36px] font-black text-black dark:text-[#F9F9FB] tracking-tight leading-tight">{t("auth.loginTitle")}</h2>
      ...
      {step === 1 ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6 px-2">
          <div>
            <label className="block text-sm font-bold text-gray-900 dark:text-[#F9F9FB] mb-2">{t("auth.emailLabel")}</label>
            <InputField type="email" placeholder={t("auth.emailLabel")} value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm font-bold text-gray-900 dark:text-[#F9F9FB] mb-2">{t("auth.passwordLabel")}</label>
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-[#A69ACA] dark:bg-[#492775] text-white font-bold rounded-full text-base px-5 py-4 mt-4 hover:bg-[#9084b8] dark:hover:bg-[#3a1f5d] transition-colors cursor-pointer border-none">
