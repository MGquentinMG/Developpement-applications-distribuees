    ...
    <div className="bg-white dark:bg-[#1A1A2E] rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.5)] w-full max-w-[400px] p-8 pt-10 relative transition-colors duration-300">
      ...
      {step === 1 ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-2">
          <div>
            <label className="block text-sm text-gray-900 dark:text-[#F9F9FB] mb-2">{t("auth.emailLabel")}</label>
            <InputField type="email" placeholder={t("auth.emailLabel")} value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm text-gray-900 dark:text-[#F9F9FB] mb-2">{t("auth.passwordLabel")}</label>
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-[#A69ACA] dark:bg-[#492775] text-white font-medium rounded-full text-sm px-5 py-3 mt-4 hover:bg-[#9084b8] dark:hover:bg-[#3a1f5d] transition-colors cursor-pointer border-none">
      ...
      <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 text-center">
        <p className="text-sm text-gray-800 dark:text-gray-300">
          {t("auth.noAccount")} <br/>
          <button onClick={onSwitchMode} className="text-[#A69ACA] dark:text-[#8B7BB5] hover:underline cursor-pointer bg-transparent border-none mt-1 font-medium">
            {t("auth.signUp")}
          </button>
        </p>
      </div>
